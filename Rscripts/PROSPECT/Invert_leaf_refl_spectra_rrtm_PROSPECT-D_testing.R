...
#   -- additional outputs: PROSPECT-5(b,D) fitted parameters N, Cab, Can, Car, Cm, Cw, ...
#   
...
#
#    --- Last updated:  03.26.2026 By Shawn P. Serbin <shawn.p.serbin@nasa.gov>
####################################################################################################
...
ok <- require(rrtm) ; if (! ok) {
  ...
  #devtools::install_github(repo = "serbinsh/rrtm", ref = "sensor_rsr")
} else {
...
# also currently required - PEcAn.assim.batch for autoburnin, need to replace with
# non pecan assim batch version
#"distributions3", 
# not in
`%notin%` <- Negate(`%in%`)
...
# get burnin functions from PEcAn
...
dataID <- "Panama2016"  #Panama2016, Panama2017, PuertoRico_2017
...
#--------------------------------------------------------------------------------------------------#
## Output directory
out.dir <- file.path(here::here(),"R_Output",dataID,"PROSPECTD_rrtm-TEST",'Range_400_700nm')
...
lines(seq(Start.wave,End.wave,1),sub_refl_data[15,])
#--------------------------------------------------------------------------------------------------#
...
#curve(dlnorm(x, 4.5, 0.45), 0, 200)
curve(dnorm(x, 65, 22), 0, 140)
...
#system.time(for (i in seq_along(1:3) ) {
  ...
  # stats
  p.refl.stats$lower[i,] <- apply(RT_pred,2,stats::quantile,probs=c(0.05), na.rm=T)
